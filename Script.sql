...
LOAD DATA INPATH '/user/raj_ops/Data/' INTO TABLE march_pageviews;
...
INSERT INTO TABLE a_en_pageviews PARTITION (lang = 'en') SELECT page, views FROM march_pageviews WHERE (lang = 'en');

INSERT INTO TABLE a_en_pageviews PARTITION (lang = 'en.m') SELECT page, views FROM march_pageviews WHERE (lang = 'en.m');
...
INSERT INTO TABLE internal_links PARTITION (type = 'link') SELECT prev, curr, occ FROM march_clickstream WHERE type = 'link';
...
SELECT a.page, a.total_views_am, g.total_views_gm FROM total_am_views a INNER JOIN total_gm_views g ON (a.page = g.page) LIMIT 10;

SELECT a.page, a.total_views_am, g.total_views_gm FROM total_am_views a INNER JOIN total_gm_views g ON (a.page = g.page) ORDER BY a.total_views_am DESC LIMIT 10;
